...
SPORTS = [
    {"str": "volleyball", "url": "47006321110679764"}, 
    {"str": "badminton", "url": "47006321112433509"}
]
...
def check_for_open_spots(sport):
    ...
    for entry in current_data:
        ...
        if class_id in previous_availability_data:
            ...
            if (not previous_sign_up_available and current_sign_up_available) or (previous_availability == "0" and current_availability != "0"):
                print(f"Spot opened up for {entry['Class Name']} ({sport['str']}) at {entry['Location']} on {entry['Date']} from {entry['Time']}!")
                Notifier.notify_users(f"===== {entry['Class Name']} =====\\n- {entry['Location']}\\n- {entry['Date']}\\n- {entry['Time']}")
